...
rule all:
	input:
		config['output_directory']+"/all_arms.tsv"
#---------------------------------------------------------------------------------------------------
...
rule create_all_arm_combinations:
	...
	threads: 20
	shell:
		"python scripts/create_all_arm_combinations.py -i {input.targets} -o {output.tsv} -c {input.config_file} -b {input.bed} -t {threads}"
